...
links.forEach(link => {
    const linkUrl = link.getAttribute('href').replace('./', '').split('/').pop().toLowerCase(); 
...
const closeButton = popup.querySelector('.popup-header a');
const closeButton2 = popup.querySelector('.popup-button .btn_popup');
const closeButton3 = popup.querySelector('.popup-button .save');

// Fungsi untuk menampilkan popup
function showPopup() {
    popup.style.opacity = 1;
    popup.style.visibility = 'visible';
    document.body.classList.add('no-scroll'); // Tambah kelas no-scroll
}
...
function hidePopup() {
    ...
    document.body.classList.remove('no-scroll'); // Hapus kelas no-scroll
}
...
closeButton2.addEventListener('click', hidePopup1);
closeButton3.addEventListener('click', hidePopup2);

// Event listener untuk tombol yang memicu popup
document.querySelector('.challenge-button').addEventListener('click', showPopup);

// Untuk menutup popup dengan klik di luar area popup
